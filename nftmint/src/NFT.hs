...
mkTokenPolicy :: POSIXTime -> TokenName -> Integer -> () -> ScriptContext -> Bool
mkTokenPolicy dl tn amt () ctx = traceIfFalse "Deadline reached"   checkDeadline         && --getinng error that oef isnt even used
  ...
  where
    ...
    checkDeadline = to mDeadline `contains` txInfoValidRange info
        where
            mDeadline ::  POSIXTime
            mDeadline =  dl --1649287216000  ---make invalid hereafter slightly before deadline
...
tokenPolicy :: POSIXTime -> TokenName -> Integer -> Scripts.MintingPolicy
tokenPolicy dl tn amt = mkMintingPolicyScript $
    $$(PlutusTx.compile [|| \dl' tn' amt' -> Scripts.wrapMintingPolicy $ mkTokenPolicy dl' tn' amt' ||])
    `PlutusTx.applyCode`
    PlutusTx.liftCode dl
    `PlutusTx.applyCode`
...
tokenCurSymbol :: POSIXTime -> TokenName -> Integer -> CurrencySymbol
tokenCurSymbol dl tn = scriptCurrencySymbol . tokenPolicy dl tn
