...
Next, ensure the time (in POSTIXtime) in the haskell code in the NFT.hs file is the same (but slightly less) than the time (in slots) in the invalid-hereafter line in mint-token-cli.sh    (86400 slots is 24 hours)
...
Next, run the command
	$ cabal exec token-policy -- policy.plutus epoch_time_deadline  desired_number_of_tokens desired_token_name
	$ cat policy.plutus  
...
Finally, use the command $./mint-token-cli.sh your_txHash#your_TxIX desired_number_of_tokens desired_token_name testnet/your.addr testnet/your.skey nftnum 
